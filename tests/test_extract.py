...
from piculet import make_rules as Rules
...
def test_extracted_value_should_be_reduced(shining):
    items = [Rule(key="title", value=Path("//title/text()", reduce=reducers.first))]
    data = Rules(items)(shining)
    assert data == {"title": "The Shining"}


def test_default_reducer_should_be_concat(shining):
    items = [Rule(key="full_title", value=Path("//h1//text()"))]
    data = Rules(items)(shining)
...
def test_added_reducer_should_be_usable(shining):
    ...
    items = [Rule(key="year", value=Path("//h1//text()", reduce=reducers.second))]
    data = Rules(items)(shining)
    assert data == {"year": "1980"}


def test_reduce_by_lambda_should_be_ok(shining):
    items = [Rule(key="title", value=Path("//title/text()", reduce=lambda xs: xs[0]))]
    data = Rules(items)(shining)
    assert data == {"title": "The Shining"}


def test_reduced_value_should_be_transformable(shining):
    items = [Rule(key="year", value=Path('//span[@class="year"]/text()', transform=int))]
    data = Rules(items)(shining)
    assert data == {"year": 1980}
...
def test_item_with_empty_str_value_should_be_included():
    ...
    items = [Rule(key="foo", value=Path("//foo/@val"))]
    data = Rules(items)(build_tree(content))
    assert data == {"foo": ""}


def test_item_with_zero_value_should_be_included():
    content = '<root><foo val="0"/></root>'
    items = [Rule(key="foo", value=Path("//foo/@val", transform=int))]
    data = Rules(items)(build_tree(content))
    assert data == {"foo": 0}
...
def test_item_with_false_value_should_be_included():
    content = '<root><foo val=""/></root>'
    items = [Rule(key="foo", value=Path("//foo/@val", transform=bool))]
    data = Rules(items)(build_tree(content))
...
def test_empty_values_should_be_excluded_from_multivalued_item_list(shining):
    items = [Rule(key="foos", value=Path(foreach='//ul[@class="foos"]/li', path="./text()"))]
    data = Rules(items)(shining)
    assert data == {}
