...
"The Shining" in our examples [#xhtml]_:
...
This function takes a document and the extraction specifications as parameters.
The document has to be well-formed XML, and it will first be converted
into a tree. Then the extraction rules will be applied to the root node
of the tree. The function returns a mapping where each item is generated
by one of the rules.

The specification is a mapping which has to contain an ``items`` key
where each item is itself a mapping. In the item mapping, the ``key`` specifies
the key for the item in the output mapping and the ``value`` specifies
how to extract the data to set as the value for that item. Typically, a value specifier consists of a path query and
...
and the reducing function ``first`` selects the first element
from that list.
...
The default reducer is ``concat``, i.e. if no reducer is given, the strings
