...
Typically, a value specifier consists of a path query and a reducing function.
The query is applied to the root and a list of strings is obtained.
Then, the reducing function converts this list into a single
string. [#reducing]_
...
from the output.
Note that in the following example, the "foo" key doesn't get included:
...
Piculet contains a few predefined reducing functions.
Other than the ``first`` reducer used in the examples above, a very common
reducer is ``concat`` which will concatenate the selected strings:
...
As explained above, if a path query doesn't match any element,
the item gets automatically excluded.
That means, Piculet doesn't try to apply the reducing function
on the result of the path query if it's an empty list.
Therefore, reducing functions can safely assume that the path result
is a non-empty list.
...
A transformation function must take a string as parameter,
...
This is a path expression to select elements from the tree. [#multivalued]_
The path and reducing function will be applied *to each selected element*,
...
To get the key value, we can use paths, reducers -and also transformers-
...
.. [#reducing]
   ...
   And the reducing function should be implemented
   so that it takes a list of strings and returns a string.
